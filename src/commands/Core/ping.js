 ...
 */
async function runPrefixPing(message) {
  ...
  try {
    ...
    // Try to edit the original "Pinging..." message instead of sending a new one
    if (pingingMessage) {
      await pingingMessage.edit({ content: null, embeds: [buildErrorEmbed('prefix')] }).catch(() => null);
      return;
    }
 ...
 */
async function runSlashPing(interaction) {
  // Guard: if the handler already replied/deferred, bail out
  if (interaction.replied || interaction.deferred) {
    logger.warn('Ping slash: interaction already handled', {
      event: 'ping.slash.already_handled',
      userId: interaction.user.id,
      guildId: interaction.guildId,
    });
    return;
  ...
  try {
    ...
    // CRITICAL FIX: use safeEditReply, NOT safeReply, because we already deferred
    try {
      await InteractionHelper.safeEditReply(interaction, {
        content: null,
...
export default {
  ...
  async prefixExecute(message) {
    return runPrefixPing(message);
  },

  async execute(interaction) {
    return runSlashPing(interaction);
  },
};  excellent: { max: 50, color: 'success', label: 'Excellent', emoji: '✨' },
  good: { max: 100, color: 'success', label: 'Good', emoji: '✅' },
  fair: { max: 250, color: 'warning', label: 'Fair', emoji: '⚠️' },
